...
/* if file uploader in html form is not empty */
	//if(isset($_FILES['file'])) {
		...
		//if(!$exist){
				...
				//$target2 = '../image-list/'.$_FILES['file']['name'];
				
				//move_uploaded_file($tempname , $target);
				//copy($target, $target2);
				
				
				if(isset($_POST['CopySku'])){
				
				
				$_SESSION['CopySku'] = $_POST['CopySku'];
				...
				echo '<script type="text/javascript"> alert("Successfully Uploaded."); window.location.href = "upload-photo.php?id='.$_SESSION['CopySku'].'";  </script>';
				
				/* insert the path of the images in database */
				
				$sql="INSERT INTO tbl_product (prod_name, quantity, org_quantity, quality, SKU, description, category, sub_category, howToUse, careInstruction, warning, sizes, colour, designs, price, listed_quantity, datetime)
				VALUE
				('$prod_name', '$quantity', '$org_quantity', '$quality', '$SKU', '$description', '$category', '$sub_category', '$howToUse', '$careInstruct', '$warning', '$size2', '$colour2', '$design2', '$price2', '$listed_quantity2', '$date')";
	...
	//}
}else{ echo '<script type="text/javascript"> alert("please Set your SKU."); </script>';}
...
<script>
function SkuChange() {
    var x = document.getElementById("SKU").value;
    document.getElementById("CopySku").value = x;
	//alert('sku coppied');
}
</script>
...
<BODY>
	...
	<center>
		<table width=670px" >
			...
			<tr><td><div><input required type="text" id="SKU" name="SKU" maxlength="15" placeholder="SKU"  onchange="SkuChange()"/><span id="user-result" style="float:right; position:relative; top:-43px; left:-10px;"></span></div></td></tr>
			<tr><td><input type="hidden" id="CopySku" name="CopySku"></td></tr>
